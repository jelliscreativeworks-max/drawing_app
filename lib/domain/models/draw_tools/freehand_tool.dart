...
class FreehandTool extends DrawTool {
  ...
  void draw(Canvas canvas, CanvasCommand drawCommand) {
    if(drawCommand is !DrawCommand) return;
  ...
  void onDrawEnd({
    ...
    if(activeCommand != null){
      activeCommand.addCommand(drawHistory: drawHistory, undoHistory: undoHistory, redoHistory: redoHistory);
    }
    // if(activeCommand != null && activeCommand is DrawCommand){
  ...
  CanvasCommand? onDrawStart({
    ...
    required String layerId,
    required List<CanvasCommand> drawHistory,
    required Map<String, List<CanvasCommand>> layerDrawHistory,
    required ToolMatrixPayload camera
  }) {
   return DrawCommand(
      toolName: toolName,
  ...
  CanvasCommand? onUpdateTool({
    ...
    if(activeCommand is !DrawCommand) return activeCommand;
